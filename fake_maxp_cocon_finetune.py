...
def split_text_into_chunks_with_special_tokens(text, tokenizer, max_length=512):    
    ...
    tokens = tokenizer.tokenize(text, max_length=4096)
...
def train(dataset_name):
    ...
    def in_batch_collate_fn(batch):
        ...
        for i in range(batch_size):
            ...
            # 긍정 샘플 추가
            input_examples.extend([InputExample(texts=[query, positive], label=1.0) for positive in positives])
            for j in range(batch_size):
                if i != j:
                    negatives = batch[j]['positives']
                    # 부정 샘플 추가
                    input_examples.extend([InputExample(texts=[query, negative], label=0.0) for negative in negatives])
    ...
    batch_size=24 if "QA" not in dataset_name else 16
    ...
    model.fit(
        ...
        output_path=f'models/{dataset_name}/fakemp_co-condenser-marco_full'
    )

    # 12. 모델 저장
    model.save(f'models/{dataset_name}/fakemp_co-condenser-marco_full')

if __name__ == "__main__":
    import os
    os.environ['CUDA_VISIBLE_DEVICES'] = "1"
    task_names = ["FinDER", "FinQABench", "FinanceBench", "TATQA", "FinQA", "ConvFinQA", "MultiHiertt"][6:]
